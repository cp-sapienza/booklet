...
\snippet{source/binomial_coefficient.h}
...
If \verb|mod| $> 2^{32}$, use \verb|mod_mul(ll, ll, ll)| instead of \verb|*| \\
\verb|init_factorials(int, int)| must be called before this function!
\snippet{source/catalan_number.h}
...
Requires \verb|egcd(ll, ll)|
\snippet{source/chinese_remainder_theorem.h}
...
\snippet{source/mobius_function.h}
...
\end{lstlisting}
\snippet{source/dnc_dp_optimization.h}

\subsection{Count inversions}
\bigo{NlogN} Counts the number of inversions in a 0-based permutation.
An inversion is a swap of two adjacent elements. \\
Requires \verb|Fenwick|
\snippet{source/inversions.cpp}
