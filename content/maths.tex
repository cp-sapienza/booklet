...
Works with \verb|int|
\snippet{source/bin_expo.cpp}

Works with \verb|long long| \\
Requires \verb|mod_mul(ll, ll, ll)|
\snippet{source/bin_expoll.cpp}
...
\snippet{source/mod_inverse.cpp}
...
\snippet{source/binomial_coefficient.cpp}
...
If \verb|mod| $> 2^{32}$, use \verb|mod_mul(ll, ll, ll)| instead of \verb|*| \\
\verb|init_factorials(int, int)| must be called before this function!
\snippet{source/catalan_number.cpp}
...
\snippet{source/pollard_rho.cpp}
...
Requires \verb|egcd(ll, ll)|
\snippet{source/diophantine.cpp}
...
Requires \verb|egcd(ll, ll)|
\snippet{source/chinese_remainder_theorem.cpp}
...
\snippet{source/mobius_function.cpp}
...
\end{lstlisting}
\snippet{source/dnc_dp_optimization.cpp}
