...
\bigo{E+V}. 
The first value in each \verb|adj| entry is the usual vertex id, the second one must be an edge id: \\
the id must be a integer in [0, E-1] and it must be the same id for both directions if the edge is undirected.\\
